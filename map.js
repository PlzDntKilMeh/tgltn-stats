...
const els = {
  ...
  dateSliderFill: document.getElementById("date-slider-fill"),
  dateRangeReadout: document.getElementById("date-range-readout"),
...
// Compact MM-DD-YY rendering for the date-range slider readout — the full
// formatEventTime() timestamp (with time-of-day) is too wide to sit under a
// narrow sidebar slider, and to-the-second precision isn't useful there
// anyway since the slider itself only resolves to whole days for a person
// dragging it.
function formatSliderDate(ms) {
...
// Single combined "MM-DD-YY – MM-DD-YY" readout plus a highlighted fill bar
// between the two thumbs, so the selected span is visible at a glance
// instead of just two independent-looking sliders stacked on top of each
// other.
function updateDateRangeLabels() {
  const from = Number(els.dateFromSlider.value);
  const to = Number(els.dateToSlider.value);
  els.dateRangeReadout.textContent = `${formatSliderDate(from)} – ${formatSliderDate(to)}`;

  const bounds = state.dateBounds;
  if (!bounds || !els.dateSliderFill) return;
  const span = bounds.max - bounds.min || 1;
  const fromPct = ((from - bounds.min) / span) * 100;
  const toPct = ((to - bounds.min) / span) * 100;
  els.dateSliderFill.style.left = `${fromPct}%`;
  els.dateSliderFill.style.width = `${Math.max(0, toPct - fromPct)}%`;
}
